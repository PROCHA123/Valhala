...
public class LineTimer{
 ...
 //--------------------------------variables---------------------------//
  ...
  public void finalLineTimer(){
    if(mGetTimeAction.getAbsoluteTimer()-mGetTimeAction.getRelativeTimer()<5){
        mForwardAction.finalMoveForwardAction(0.8);
    }
    else if(mGetTimeAction.getAbsoluteTimer()-mGetTimeAction.getRelativeTimer()<5.5){
    ...
    while(!BLimitSwitch.get()&&(mGetTimeAction.getAbsoluteTimer()-mGetTimeAction.getRelativeTimer()<13)){
      ...
      mForwardAction.finalMoveForwardAction(0.6); 

        }
    }
    else if(caja&&(mGetTimeAction.getAbsoluteTimer()-mGetTimeAction.getRelativeTimer()<15)){
          mForwardAction.finalMoveForwardAction(0.3);   
    }
    else{
      mStopAction.finalStopAction();
    }
